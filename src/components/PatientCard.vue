    ...
    <template v-if="mode === 'queue'">
      <div class="p-2.5">
        ...
        <div class="flex items-center gap-1.5 mb-1">
          <span class="text-[11px] font-bold px-1.5 py-0.5 rounded shrink-0" :class="urgencyBadge(task.urgency)">
            {{ urgencyLabel(task.urgency) }}
          </span>
          <span class="text-sm font-bold text-gray-100 truncate">{{ task.patient_name }}</span>
          <span class="text-[13px] text-gray-400 shrink-0">({{ task.gender }}/{{ task.age }})</span>
        ...
        <div class="text-[13px] font-semibold text-gray-200 mb-1 line-clamp-1 border-l-2 border-gray-600 pl-2">
        ...
        <div class="flex items-center justify-between text-[10px] text-gray-400">
          <span class="truncate max-w-[65%]" :title="task.diagnosis">
            <i class="fa-solid fa-stethoscope text-[10px] mr-1 text-gray-600"></i>{{ task.diagnosis }}
          </span>
          <span class="shrink-0 text-gray-300">
            <i class="fa-solid fa-user-md text-[13px] mr-1 text-gray-500"></i>{{ task.surgeon }}
          </span>
        </div>
        ...
        <div v-if="task.vs_note" class="mt-1.5 flex items-start gap-1.5 bg-yellow-900/30 border border-yellow-700/50 rounded px-2 py-1">
          ...
          <span class="text-[11px] text-yellow-300 leading-snug break-all">{{ task.vs_note }}</span>
        </div>
      ...
      <div class="p-1.5 h-full flex flex-col overflow-hidden leading-tight">
        <div class="flex items-center justify-between mb-0.5 shrink-0">
          <div class="flex items-center gap-1 truncate">
            <span class="text-[13px] font-bold text-gray-100">{{ task.patient_name }}</span>
            <span class="text-[11px] text-gray-400">({{ task.bed_no }})</span>
          </div>
          <div class="flex items-center gap-0.5 shrink-0">
            <span v-if="task.linked_task_id" class="text-[10px] px-1 rounded bg-teal-800 text-teal-200" title="兩科合刀">🔗</span>
            <span v-if="task.status === 'called'"     class="text-[10px] font-bold px-1 rounded bg-amber-700 text-amber-100">叫</span>
            <span v-if="task.status === 'in_surgery'" class="text-[10px] font-bold px-1 rounded bg-green-700 text-green-100 animate-pulse">進</span>
            <span v-if="task.status === 'recovery'"   class="text-[10px] font-bold px-1 rounded bg-purple-700 text-purple-100">完</span>
            <span class="text-[10px] font-bold px-1 rounded scale-90" :class="urgencyBadge(task.urgency)">
              {{ urgencyLabel(task.urgency) }}
            </span>
          </div>
        </div>
        <div class="text-[11px] text-blue-200 truncate font-medium shrink-0">{{ task.procedure }}</div>
        <div class="text-[10px] text-gray-400 truncate mt-0.5 shrink-0">
          <i class="fa-solid fa-user-md text-[9px] mr-0.5"></i>{{ task.surgeon }}
        </div>
        <div v-if="task.vs_note" class="text-[10px] text-yellow-500/80 truncate mt-auto italic">
          <i class="fa-solid fa-comment-medical text-[9px] mr-0.5"></i>{{ task.vs_note }}
        </div>
      </div>
    </template>
  </div>

  <!-- Hover tooltip: full card info -->
  <Teleport to="body">
    <div
      v-if="hoverTooltip && !isDraggingThis"
      class="fixed z-[9990] pointer-events-none w-60"
      :style="{ left: tooltipX + 'px', top: tooltipY + 'px' }"
    >
      <div class="rounded-lg border shadow-2xl bg-gray-800 border-gray-600 p-2.5">
        <div class="flex items-center gap-1.5 mb-1">
          <span class="text-[10px] font-bold px-1.5 py-0.5 rounded shrink-0" :class="urgencyBadge(task.urgency)">{{ urgencyLabel(task.urgency) }}</span>
          <span class="text-xs font-bold text-gray-100 truncate">{{ task.patient_name }}</span>
        ...
        <div class="flex items-center gap-1.5 text-[10px] mb-1.5">
          ...
          <span class="text-orange-300">{{ task.bed_no || '無床號' }}</span>
          <div class="flex-1" />
          <span v-if="task.expected_room" class="bg-blue-900/40 text-blue-300 px-1 rounded border border-blue-800/50">
            <i class="fa-solid fa-door-open text-[8px] mr-1"></i>{{ task.expected_room }}
          </span>
        </div>
        <div class="text-[10px] font-semibold text-gray-200 mb-1 line-clamp-2 border-l-2 border-gray-600 pl-2">
          {{ task.procedure || '（未填手術名稱）' }}
        </div>
        <div class="flex items-center justify-between text-[10px] text-gray-400 mb-1">
          <span class="truncate max-w-[60%]"><i class="fa-solid fa-stethoscope text-[9px] mr-1 text-gray-600"></i>{{ task.diagnosis }}</span>
          <span class="shrink-0 text-gray-300"><i class="fa-solid fa-user-md text-[9px] mr-1 text-gray-500"></i>{{ task.surgeon }}</span>
        </div>
        <div v-if="task.vs_note" class="flex items-start gap-1 bg-yellow-900/30 border border-yellow-700/50 rounded px-2 py-1">
          <i class="fa-solid fa-triangle-exclamation text-yellow-400 text-[9px] mt-0.5 shrink-0"></i>
...
function onMouseEnter(e: MouseEvent) {
  hoverTimer = setTimeout(() => {
    ...
    const x = rect.right + 8 + TW > vw ? rect.left - TW - 8 : rect.right + 8;
    const y = rect.top + TH > vh ? vh - TH - 8 : rect.top;
    tooltipX.value = Math.max(8, x);
    tooltipY.value = Math.max(8, y);
