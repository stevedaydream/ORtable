...
pub async fn upsert(pool: &SqlitePool, rule: &DeptRule) -> Result<DeptRule, String> {
    let rooms_json = serde_json::to_string(&rule.preferred_rooms)
        .map_err(|e| e.to_string())?;
    sqlx::query(
        "INSERT INTO dept_rules (dept, priority_bonus, preferred_rooms)
         VALUES (?, ?, ?)
         ON CONFLICT(dept) DO UPDATE SET priority_bonus=excluded.priority_bonus, preferred_rooms=excluded.preferred_rooms",
    )
    .bind(&rule.dept).bind(rule.priority_bonus).bind(&rooms_json)
...
pub async fn replace_all(pool: &SqlitePool, rules: &[DeptRule]) -> Result<(), String> {
    ...
    for r in rules {
        ...
        sqlx::query("INSERT INTO dept_rules (dept, priority_bonus, preferred_rooms) VALUES (?,?,?)")
            .bind(&r.dept).bind(r.priority_bonus).bind(&rooms)
            .execute(&mut *tx).await.map_err(|e| e.to_string())?;
    }
