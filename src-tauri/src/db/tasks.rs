...
pub async fn create(pool: &SqlitePool, task: &SurgeryTask) -> Result<SurgeryTask, String> {
    ...
    let id = sqlx::query(
        "INSERT INTO surgery_tasks
         (seq_no,patient_name,gender,age,chart_no,bed_no,dept,diagnosis,body_part,procedure,anesthesia,surgeon,vs_note,expected_room,urgency,scheduled_at,created_at,est_time_mins,status,linked_task_id)
         VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
    )
    ...
    .bind(task.est_time_mins as i64).bind(&task.status).bind(task.linked_task_id)
...
pub async fn update(pool: &SqlitePool, task: &SurgeryTask) -> Result<SurgeryTask, String> {
    sqlx::query(
        "UPDATE surgery_tasks SET
         ...
         scheduled_at=?,est_time_mins=?,status=?,linked_task_id=?
         WHERE id=?",
    )
    .bind(task.seq_no).bind(&task.patient_name).bind(&task.gender).bind(task.age)
    .bind(&task.chart_no).bind(&task.bed_no).bind(&task.dept).bind(&task.diagnosis)
    ...
    .bind(&task.status).bind(task.linked_task_id).bind(task.id)
    .execute(pool).await.map_err(|e| e.to_string())?;
...
pub async fn replace_all(pool: &SqlitePool, tasks: &[SurgeryTask]) -> Result<(), String> {
    ...
    for t in tasks {
        sqlx::query(
            "INSERT INTO surgery_tasks
             (id,seq_no,patient_name,gender,age,chart_no,bed_no,dept,diagnosis,body_part,procedure,anesthesia,surgeon,vs_note,expected_room,urgency,scheduled_at,created_at,est_time_mins,status,linked_task_id)
             VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
        )
        ...
        .bind(t.est_time_mins as i64).bind(&t.status).bind(t.linked_task_id)
        .execute(&mut *tx).await.map_err(|e| e.to_string())?;
    }
        ...
        sqlx::query(
            "INSERT INTO surgery_tasks
             (seq_no,patient_name,gender,age,chart_no,bed_no,dept,diagnosis,body_part,procedure,
              anesthesia,surgeon,vs_note,expected_room,urgency,scheduled_at,created_at,est_time_mins,status,linked_task_id)
             VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
        )
